...
class Case():
    def __init__(self):
        self.get = Rect(121, 386, 318, 449)  # 领取任务
        self.ok = Rect(1110, 807, 1451, 875)  # 领取完毕
        self.start_mission = Rect(1085, 1309, 1461, 1395)  # 开始任务
        self.point = Rect(1500, 1160, 1567, 1223)
        self.safe_point = Rect(2355, 913, 2476, 980)  # 点击此处无意义
        self.threshold = 0.8
    ...
    def run(self):
        ...
        screen = screenshot()
        patch = self.get.patch_on_img(screen)
        ssim_get = skm.compare_ssim(patch, self.get_img, multichannel=True)
        logging.info(f"当前任务是否已经结束: ssim get:\t{ssim_get:.4f}")
        if ssim_get >= self.threshold:
            click(self.get)  # 领取任务
            click(self.ok)  # 领取完毕
            click(self.safe_point)

            click(self.point)  # 点击任务点
            screen = screenshot()
            patch = self.start_mission.patch_on_img(screen)
            ssim_start = skm.compare_ssim(patch, self.start_img, multichannel=True)
            logging.info(f"当前是否处于开始任务: ssim start:\t{ssim_start:.4f}")
            click(self.start_mission)  # 开始任务
            click(self.safe_point)
...
if __name__ == "__main__":
    time.sleep(3)  # 预留时间切换到挂机界面
    # map_next_refresh_time = "04:20:00"
    map_next_refresh_time = args.time
    main = Main(map_next_refresh_time)
    main.run()
